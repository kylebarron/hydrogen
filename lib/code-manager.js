...
export function getSelectedText(editor: atom$TextEditor) {
  ...
  const grammarId = editor.getGrammar().id;
  const stripGrammars = Config.getJson("stripIndentBeforeSendToKernel") || [];
  if (_.includes(stripGrammars, grammarId) && selectedText) {
    return stripCommonLeadingWhitespace(selectedText);
  }
  return normalizeString(selectedText);
}
...
function stripCommonLeadingWhitespace(code: ?string) {
  code = code.replace(/\r\n|\r/g, "\n");
  if (!code.includes("\n")) {
    return code.trim();
  }

  const lines = _.without(code.split("\n"), "");
  const leadingWhitespace = lines[0].match(/^\s+/);
  if (!leadingWhitespace) {
    return code.trim();
  }
  const re = new RegExp("^(" + leadingWhitespace[0] + ")");
  var linesDedented = [];
  for (var i = 0; i < lines.length; i++) {
    let text = lines[i];
    text = text.replace(re, "");
    if (text !== lines[i]) {
      linesDedented.push(text);
    } else {
      atom.notifications.addError(
        "Leading whitespace not common to all lines",
        {
          description:
            "All lines in selection must have at least as much leading whitespace as the first line."
        }
      );
      return "";
    }
